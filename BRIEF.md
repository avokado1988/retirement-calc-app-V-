...
> **ענף בסיס:** `track4-graphs-v3`. הענף `claude/final-v4` נחשב לא יציב ואין לבסס עליו. תיקונים חדשים על `BugFix-July-5-V5`.

---

## 🎩 כובע מקצועי — חובה בכל תשובה מקצועית

בפרויקט הזה קלוד לובש שני כובעים בו-זמנית: מפתח, וגם מתכנן פיננסי מוסמך (CFP) ויועץ בכיר לפרישה, השקעות ופנסיה בישראל, שמבקר את המחשבון מבחינה מקצועית.

כל שאלה בהיבט המקצועי חייבת להיבחן דרך עיני מומחה פרישה, ולא רק דרך נכונות הקוד. חובה:

- לחזור ולאמת את מספרי המשיכה והצמיחה מול נתוני בנצ'מרק, אינפלציה, תוחלת חיים ו-Best Practices של השוק הישראלי
- לבחון סבירות אקטוארית: שיעורי משיכה בטוחים, כלל 4%, מקדמי המרה, תשואות ריאליות, דמי ניהול, הצמדות מדד
- להתייחס למיסוי הישראלי (תיקון 190, מס ריאלי 25%, מיסוי שכירות) ולהתאמתו למציאות
- לציין במפורש כשמספר או הנחה חורגים מהמקובל בשוק, גם כאשר הקוד תקין טכנית

המסקנה: קלוד כאן הוא יועץ מקצועי של התחום, לא רק יועץ קוד.

---

## 🆕 מצב עדכני — מה נבנה בסשן יולי 2026 (חשוב לקרוא)

הענף `BugFix-July-5-V5` מכיל שדרוגים משמעותיים מעבר לתיאור ההיסטורי שבהמשך המסמך:

**חמישה מסלולים (לא ארבעה).** נוסף **מסלול 5 — מינוף (הלוואת בלון)**: קונים את הדירה החדשה בהלוואת בלון כנגד תיק ההשקעות במקום במזומן, כך שכסף הדירה נשאר מושקע. תיק = תיק מסלול 1 + סכום ההלוואה. מיסוי ותשואה כמו תיקון 190. החוב תופח בריבית (בלי תשלום חודשי, recourse, לא מוגבל) ונפרע מהעיזבון. חוגה אחת: הלוואה=0 זהה למסלול 1, עד מחיר הדירה=מינוף מלא. קלט ב-`inputs/leverage.py`, סעיף 9 בסיידבר. עמודות מנוע: `צבירה מסלול מינוף`, `הלוואת בלון — יתרת חוב/ריבית`, `מינוף — LTV`.

**משכנתה הפוכה אוטומטית (מסלול 4).** אין יותר הפעלה ידנית. מושכים מהתיק עד רצפת מזומן `rm_savings_floor`=100K, ואז RM מכסה אוטומטית את הגרעון עד הגיל הנבדק, בריבית `rm_annual_rate`=6%, מוגבל ב-LTV לפי גיל, ו-non-recourse (חוב תפוס לשווי הנכס). גרעון שה-RM לא מכסה ⇒ מסלול 4 "לא קביל".

**עזרה לילדים כנכס משפחתי צומח.** שדה `kids_help_growth` (5%). הכסף לא נעלם — במסלולים 1,2,3,5 (מכירה) הוא חוזר כשורה בכרטיס וב-`sa_100` (הדירוג), צומח לגיל הנבדק. במסלול 4 לא (שומרים את הנכס).

**מס שבח עתידי על מסלול 4.** אומדן מאחורי הקלעים (בסיס ₪2M, רכישה 2006, פטור לינארי, 25% ריאלי, מכויל לעוגן ~₪900K של יועץ המס), מנוכה מסך הנכסים של מסלול 4. פינוי בינוי: הבסיס מקורי ונמוך, בלי ניכוי בנייה — המס תופח לעתיד.

**מונטה קרלו.** לשונית `🎲 סיכון מונטה קרלו` (`reports/monte_carlo.py`) — אלפי תרחישי שוק, ולכל גודל הלוואה: הסתברות דרישת ביטחונות + התפלגות ירושה (10/50/90). זה מנוע ההחלטה למסלול המינוף. סליידרים לתנודתיות ולסף מכירה.

**כרטיסי סיכום מנהלים חדשים.** שורות אחידות מיושרות: חלק "קיימות התיק" (השלמה חודשית בפרישה, גיל תחילת שחיקה / "צומח תמיד", עד איזה גיל התיק מספיק + רמז), וחלק "הון בגיל הנבדק" (תיק, נדל"ן, הלוואות/מס במינוס, עזרה לילדים, סך נכסים). מד סיכון מינוף על כרטיס מסלול 5. הכל בגיל הנבדק (`check_age`), לא 100/105.

**גיל הבדיקה אוחד.** כל המקומות קוראים `DEFAULTS["check_age"]` (מקור אמת יחיד). המסלול המומלץ = חוסן עד `check_age`; שימור הון הוגן למסלולי קצבה.

**שמירה לקוד + GitHub.** כפתור "שמור כברירת מחדל" ב-`app.py` כותב את בלוק `DEFAULTS` ל-`inputs/ui_components.py` ודוחף לגיטהאב (Git Data API, דורס את קומיט השמירה הקודם). דורש `github_token` ב-Streamlit Secrets. `DEFAULTS` הוא התרחיש השמור; `visible_tracks` נשמר גם הוא.

**סביבת עבודה / דחיפות.** אין checkout מקומי אצל המשתמש — האפליקציה רצה ב-Streamlit Cloud מהענף. דחיפות של קלוד עוברות דרך טוקן ב-osxkeychain (לא בצ׳אט). שמירות מהאפליקציה יוצרות קומיטים במקביל — **לפני כל push צריך `git fetch` + `git rebase`**.
