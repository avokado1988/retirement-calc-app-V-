...
**`claude/final-v4`** — זהו ה-default branch הנוכחי (יולי 2026).  
הוא מבוסס על `track4-graphs-v3` + קבצי תיעוד (BRIEF.md, CLAUDE.md).  
לפני כל עבודה: `git checkout claude/final-v4 && git pull`
...
- `simulator_engine.py` שורה 14, `qa_report.py` שורה 31, `qa_summary.py` שורה 24: `timeline.get("check_age", 87.0)` — ה-fallback הוא 87.0 אך ברירת המחדל המתועדת היא **97.0**. משפיע רק אם `timeline` לא כולל `check_age` (מצב שלא אמור לקרות בזרימה הרגילה).
