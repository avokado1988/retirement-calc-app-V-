...
def render_timeline_inputs():
    ...
    start_age = compact_number_input(
        ...
        value=DEFAULTS["start_age"], min_value=55.0, max_value=80.0, step=0.5, unit=None
    )

    retirement_age = compact_number_input(
        "גיל פרישה (הפסקת עבודה)",
        value=DEFAULTS["retirement_age"], min_value=60.0, max_value=80.0, step=0.5, unit=None
    )

    check_age = compact_number_input(
        "גיל יעד לבדיקת סימולציה",
        value=DEFAULTS["check_age"], min_value=70.0, max_value=120.0, step=1.0, unit="שנים"
    )
