...
from inputs.ui_components import compact_number_input, COLOR_RED, COLOR_BLUE, DEFAULTS
...
def render_expenses_inputs():
    ...
    current_expenses = compact_number_input(
        ...
        value=DEFAULTS["current_expenses"], min_value=0, step=500, unit="₪", color=COLOR_RED
    )

    expected_inflation = compact_number_input(
        "אינפלציה שנתית צפויה (%)",
        value=DEFAULTS["expected_inflation"] * 100, min_value=0.0, max_value=10.0, step=0.1, unit="%", color=COLOR_BLUE
    )
    ...
    age_75_85_increase = compact_number_input(
        ...
        value=DEFAULTS["age_75_85_increase"] * 100, min_value=0.0, max_value=5.0, step=0.1, unit="%", color=COLOR_BLUE
    )

    age_85_plus_increase = compact_number_input(
        "תוספת הוצאות שנתית מגיל 85 ואילך (%)",
        value=DEFAULTS["age_85_plus_increase"] * 100, min_value=0.0, max_value=5.0, step=0.1, unit="%", color=COLOR_BLUE
    )
    ...
    one_time_expense = compact_number_input(
        ...
        value=DEFAULTS["one_time_expense"], min_value=0, step=5000, unit="₪", color=COLOR_RED
    )

    one_time_frequency = compact_number_input(
        "תדירות ההוצאה החד-פעמית (כל כמה שנים)",
        value=DEFAULTS["one_time_frequency"], min_value=1, max_value=20, step=1, unit="שנים", color=COLOR_BLUE
    )
    ...
    caregiver_cost = compact_number_input(
        ...
        value=DEFAULTS["caregiver_cost"], min_value=0, step=500, unit="₪", color=COLOR_RED
    )
