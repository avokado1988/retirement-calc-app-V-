...
def render_wealth_inputs():
    ...
    property_appreciation_pct = compact_number_input(
        ...
        help_text=("מה שקובע הוא הפרמיה מעל האינפלציה. דירת מגורים חדשה בצפון תל אביב מנצחת "
                   "אינפלציה בכאחוז וחצי ריאלי לטווח ארוך, נטו אחרי שהמבנה מתיישן. עם אינפלציה "
                   "2.3%, זה יוצא כ-3.8%. אם תשנה את האינפלציה, התאם כדי לשמור על אותה פרמיה של "
                   "כאחוז וחצי. הערך תואם לטבלת המומלצים בטאב התמהיל.")
    )
    st.caption("3.8%, כלומר כאחוז וחצי מעל האינפלציה. דירה חדשה בצפון תל אביב, נטו אחרי התיישנות. תואם לטבלת המומלצים בטאב התמהיל.")
