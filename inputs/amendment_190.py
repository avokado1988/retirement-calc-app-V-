...
def render_190_inputs(remaining_for_gimel):
    ...
    desired_pension = compact_number_input(
        ...
        value=DEFAULTS["desired_pension"], min_value=0, step=50
    )
    ...
    securing_years = labeled_slider_with_value(
        ...
        min_value=0, max_value=35, value=20, step=1
    )
    base_coefficient = labeled_slider_with_value(
        "מקדם המרה בסיסי לקצבה (ללא אבטחה)", 
        min_value=150.0, max_value=300.0, value=200.0, step=1.0, format="%.1f"
    )
    ...
    with col2:
        st.metric(label="הון נדרש שינוכה לטובת הקצבה", value=format_shekel(capital_for_pension))
    
    net_for_190 = max(0, remaining_for_gimel - capital_for_pension)
    
    if remaining_for_gimel < capital_for_pension:
        st.error(f"⚠️ אזהרה: ההון הנדרש לקצבה ({format_shekel(capital_for_pension)}) גבוה מסך ההון הזמין שלך ({format_shekel(remaining_for_gimel)})!")
    else:
        show_net_summary(title="יתרת הון נטו פנויה למשיכה הונית בתיקון 190", amount=net_for_190)
