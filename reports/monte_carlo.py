...
def render_monte_carlo(user_inputs):
    ...
    if cur_loan <= 0:
        ...
        # --- הארביטראז' — מוסתר בדרופ דאון, האקשן הוא הקלף והפסק שלמעלה ---
        ...
        with st.expander("📖 המרווח (ארביטראז') — למה התוספת יוצאת כמו שהיא"):
            _rtl(
                f"<div style='line-height:1.9;color:#2a3346;'>"
                ...
                f"ובניכוי עלות ההזדמנות, הוא התוספת בחציון "
                f"(<b>{'+' if up50>=0 else ''}{_f(up50)}</b>) מול {_alt_lbl}, ובתרחיש הגרוע "
                f"({'+' if up10>=0 else ''}{_f(up10)}), שבקלף שלמעלה.</span></div>")
    ...
    with st.expander("📉 בכמה התיק יכול לרדת — לכל גודל הלוואה", expanded=True):
        ...
        st.caption("ירידת תיק שקולה לכפליים בשוק המניות (מסלול כללי כמחצית מניות). ההסבר המלא והשכיחות ההיסטורית בדרופ דאון למטה.")

    with st.expander("📖 איך לקרוא את טבלת הירידות, ומה הסיכוי ההיסטורי"):
        _rtl(
            "<div style='line-height:1.7;color:#444;'>"
            "<b>בכמה התיק יכול לרדת</b> — עד שהחוב משתווה לשווי התיק. מתחת לזה התיק לבדו כבר לא "
            "מכסה את ההלוואה, והיתרה נאכלת מהעיזבון.<br/>"
            "<b>תרגום לשוק מניות</b> — מסלול כללי הוא כמחצית מניות, אז ירידת תיק שקולה לכפליים "
            "בשוק. ירידת תיק של 28% דורשת מפולת של כ-56% במניות.<br/>"
            "<b>סיכוי שיקרה</b> — כמה מתרחישי המונטה קרלו הגיעו לנקודה עד הגיל הנבדק, כולל תפיחת החוב.<br/><br/>"
            "📅 <b>שכיחות היסטורית:</b> מפולת מניות של ~20% כל 4-5 שנים, ~35% כל עשור, ~50%+ כארבע "
            "פעמים במאה (1929, 1974, 2002, 2008). התיק המגוון יורד כמחצית, אז צריך מפולת קיצונית "
            "וממושכת כדי להגיע לשיעורים שבטבלה.</div>")
