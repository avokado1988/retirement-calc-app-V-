...
with tab3:
    ...
    with track_tabs[3]:
        cols = COMMON_COLS + ["הכנסת שכירות נטו", "הוצאת שכירות", "צבירה מסלול שכירות", "מס ששולם שכירות", "שווי נדלן מסלול 4"]
        fmt  = {**COMMON_FMT, "הכנסת שכירות נטו": "{:,.0f} ₪", "הוצאת שכירות": "{:,.0f} ₪",
                "צבירה מסלול שכירות": "{:,.0f} ₪", "מס ששולם שכירות": "{:,.0f} ₪",
                "שווי נדלן מסלול 4": "{:,.0f} ₪"}
        st.dataframe(df_display[[c for c in cols if c in df_display.columns]].style.format(fmt), use_container_width=True)
