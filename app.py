...
# 1. הגדרת תצורת דף אחידה - חובה להיות הפקודה הראשונה!
...
# 🟢 פתרון באג הקריסה (Infinite Recursion): שומרים את הפונקציה המקורית רק פעם אחת!
if not hasattr(st, "_orig_slider_saved"):
    st._orig_slider_saved = st.slider
    st._orig_number_input_saved = st.number_input
...
def patched_slider(label, *args, **kwargs):
    ...
    val = st._orig_slider_saved(label, *args, **kwargs) # שימוש בשמירה המוגנת
...
def patched_number_input(label, *args, **kwargs):
    ...
    val = st._orig_number_input_saved(label, *args, **kwargs) # שימוש בשמירה המוגנת
    st.query_params[widget_key] = str(val)
    return val

# חטיפת הרכיבים הגלובלית - מוחל בביטחה
